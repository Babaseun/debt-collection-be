...
class Client(models.Model):
    client_reference_no = models.CharField(max_length=255, primary_key=True) # Client reference to multiple accounts obtained from csv
...
class Account(models.Model):
    ...
    status = models.CharField(max_length=50, choices=[(item.value, item.name) for item in StatusType], null= True) # TODO
